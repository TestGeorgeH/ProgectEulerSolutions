...
primes = sieve [2 ..]
  where
    sieve (p : xs) = p : sieve (filter (`notDivisibleBy` p) xs)
    sieve [] = []
...
rotateNumber n = numHead + numTail
  where
    numHead = (n `rem` 10) * 10 ^ (numDigits n - 1)
    numTail = (n `div` 10)
